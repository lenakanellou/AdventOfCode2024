...
def try_op(left, right, result):
    ...
    if len(right) == 1:
        ...
        if res == result:
            return True
        res = int(str(left)+right[0])
        print(f'Concatenating {left} and {right[0]}: {res}')
        if res == result:
            return True
        else:
            return False
        ...
        if try_op((left*int(right[0])), right[1:], result) == True:
            return True
        print(f'Concatenating {left} to {right[0]}:')
        if try_op(int(str(left)+(right[0])), right[1:], result) == True:
            return True
        return False
